...
const BoardItem = ({ board, activeBoard, setActiveBoard }: BoardItemProps) => {
	...
	return (
		<>
			<div
				className={`cursor-pointer group w-full h-12 ${
					activeBoard === board._id ? 'bg-[#635FC7]' : 'bg-transparent'
				} flex items-center pl-8 gap-4 rounded-tr-[100px] rounded-br-[100px] ${
					activeBoard !== board._id && 'hover:bg-white'
				}`}
				...
				<p
					className={`font-bold text-[0.9375rem] leading-5 capitalize ${
						activeBoard === board._id ? 'text-white' : 'text-[#828FA3]'
					} ${activeBoard !== board._id && 'group-hover:text-[#635FC7]'}`}>
					{board.name}
				</p>
			</div>
